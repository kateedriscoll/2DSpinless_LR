...
def CreateDisorderHam( Disorder ) :
    ...
    for statei, state in enumerate( config.Basis ) :
        ...
        data[ statei ] +=  Disorder * Potential
...
def CreateNearestNeighborHam( V ) :
    row = numpy.array( range( len( config.Basis ) ) )
    col = numpy.array( range( len( config.Basis ) ) )
    data = numpy.zeros( ( len( config.Basis ), ) )
    ...
    for statei, state in enumerate( config.Basis ) :
        ...
        for s in range( len( state ) ) :
            for n in range( config.Connectivity ) :
                Potential += 0.5 * V * state[ s ] * state[ config.Neighbors[ s, n ] ]
...
def CreateLongRangeHam( V ) :
    ...
    for statei, state in enumerate( config.Basis ) :
        ...
        data[ statei ] += 0.5 * V * numpy.sum( config.MatrixDist * ninj )
